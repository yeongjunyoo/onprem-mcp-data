...
> 사전: `docker compose up -d` + 모델 캐시(qwen2.5:7b, bge-m3) + `npm run gen:bench` + `EMBEDDER=ollama npm run embed:bench` + `bash scripts/replica-spike.sh` 1회(로그 확보). 녹화 직전 **네트워크 차단**(외부 API 없음 증명). 화면 우상단에 네트워크 off + Apple M4/macOS 표시.
>
...
| 0:22–0:45 | `EMBEDDER=ollama npm run demo` 실행, 도구 목록 출력 + 라우터를 **연달아 2번** 호출해 동일 결과 | "air 프레임워크 위 **7개 MCP 도구**. 도구 선택은 **규칙 기반 라우터** — LLM 호출도, 튜닝 파라미터도 없습니다. 두 번 돌려도 결과가 완전히 같죠. **튜닝 0, 분산 0.** 리원에이스가 말한 '설정 민감성 제거'를 코드로 실증합니다." |
...
## 녹화 체크리스트
- [ ] 네트워크 차단 후 `npm run demo` 1회 리허설(캐시된 모델로 통과, 20분 내).
...
- [ ] 하드웨어/OS 표시(Apple M4 / macOS 25.5), 네트워크 off 표시 상시 노출.
