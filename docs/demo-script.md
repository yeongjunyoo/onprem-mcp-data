...
**GPU 호스트 Ollama를 쓰면** 그 대기가 947ms로 줄지만 환경이 문서 수치와 달라진다 —
...
- [ ] **지연은 환경에 종속된다.** GPU 호스트 Ollama는 중앙값 947ms, GPU 패스스루 없는 컨테이너는 중앙값 15746ms(약 14초, 반복 실측 9.8~18.5초)다. 화면에 뜨는 대기 시간이 문서 수치와 다르면 어느 환경인지 자막으로 밝힌다.
