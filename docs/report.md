...
| 중국어 혼입 2건, 「dept_id 5번」 | 7B code-switching, id 그대로 노출 | 답변 프롬프트에 한국어 고정, 관계줄 해석, id 대신 이름, 동점 전부 나열. **2026-08-19: 모델을 `qwen2.5-coder:7b` 로 바꾼 뒤 한국어 고정 줄을 빼고 30문항을 다시 돌려 한자 혼입 0건을 확인했다** — 이 모델에서는 원 결함이 재현되지 않는다. 그래도 그 줄은 유지한다: 30문항 한 번으로 「절대 안 샌다」를 말할 수 없고 비용이 0이며 실패 비용은 크다 |
