...
**그 함정이 실제로 작동한 기록 (2026-08-19).** 모델을 바꾸며 답변 30건을 나란히 놓고
비교하다 개체 접지 검사의 사각을 **실측했다.**
...
데이터베이스에 직접 물으면 `Product-D1` 의 그래프 이웃은 **client 16 · project 0** 이다.
**프로젝트는 애초에 없다.** 옛 답은 **다른 종류의 개체로 다른 질문에 답한 것**인데,
그 개체들이 컨텍스트 안에 있었으므로 **개체 접지 검사를 통과했다** — 당시 접지율
19/19 = 100% 안에 이 답이 들어 있었다.
