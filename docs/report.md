...
| **NL2SQL** (재시도 없음) | execution match, 큐레이션 스키마카드 | **7/10** (2026-08-19 `qwen2.5-coder:7b`, 3회 반복 동일 · 옛 `qwen2.5:7b` 는 5~7/10 로 세션 간 변동, §0.10) | gold SQL 실행 결과 = DB (LLM 저지 없음) |
| **NL2SQL** (재시도 없음, ablation) | execution match, 테이블명만 | **1/10 = 10%** (3회 반복 동일) → **Δ +60pp** | 동일 |
