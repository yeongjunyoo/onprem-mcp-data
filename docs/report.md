...
> _배점 확정 반영(2026-08-20 공식 공지 「심사 기준 및 배점 안내」, osscontest.kr/notice/41): 1차 서면 30점 + 2차 발표 70점. 항목별 대응은 §11._
...
| **NL2SQL** (재시도 없음) | execution match, 큐레이션 스키마카드 | **7/10** (2026-08-19 `qwen2.5-coder:7b`, 5회 반복 동일 · 옛 `qwen2.5:7b` 는 5~7/10 로 세션 간 변동, §0.10) | gold SQL 실행 결과 = DB (LLM 저지 없음) |
| **NL2SQL** (재시도 없음, ablation) | execution match, 테이블명만 | **1/10 = 10%** (5회 반복 동일) → **Δ +60pp** | 동일 |
| **NL2SQL** (self-repair 1회) | 스키마카드 **7/10** / 테이블명만 **5/10** | **Δ +20pp 가 남고** 재시도는 **0회 대 7회** — 스키마 카드를 주면 이 모델은 수리가 필요 없다. 옛 `qwen2.5:7b` 에서는 7~8/10 대 7/10 으로 Δ가 0~+10pp 까지 좁혀졌고 재시도가 2회 대 6회였다(§0.6) | 동일 |
...
2. **NL2SQL 60%의 ablation이 본 프로젝트의 논지를 사업자 데이터에서 재현한다 — 옛 `qwen2.5:7b` 에서는 재시도를 붙이면 그 논지가 무너졌지만 `qwen2.5-coder:7b` 에서는 재시도를 붙여도 격차가 남는다(§0.6).** 테이블명만 준 베이스라인의 실패 8건 중 6건이 **존재하지 않는 컬럼 환각**(`contracts.is_active`, `clients.registration_date`, `employees.department_id` …)이었다. 값 어휘(`quarter='2025-Q3'`, `status='active'`)까지 담은 스키마 카드를 주면 그 실패군이 사라진다. 내부 벤치의 Δ +51.0pp와 같은 방향이며, **같은 데이터·같은 모델·같은 채점기**로 측정했다.
...
> **2026-08-19 갱신 — 이 절의 결론은 뒤집혔다.** 생성 모델을 `qwen2.5:7b` 에서 `qwen2.5-coder:7b` 로 바꾸고 같은 실험을 다시 돌리니 **차이가 사라지지 않는다**: 재시도를 붙여도 **7/10 대 5/10** 이고 수리 횟수는 **0회 대 7회** 다. 스키마 카드를 주면 이 모델은 **수리가 아예 필요 없다.** 아래 서술과 표는 `qwen2.5:7b` 에서 관측한 그대로 남긴다 — **지우지 않는다.** 자기 반증을 한 번 했고 그 반증이 다시 뒤집혔으며, 두 번 다 우리가 재서 알았다. **결론이 모델에 의존한다는 것을 실측으로 보인 사례**로 읽어 주기 바란다.

실패한 SQL을 **데이터베이스 자신의 카탈로그**(information_schema 실컬럼 목록)와 함께 모델에 한 번 되먹이는 self-repair를 붙이자, `qwen2.5:7b` 에서는 스키마 카드 유무의 **정확도 차이가 사라졌다**.
...
**해석을 바꿔야 한다.** 스키마 카드와 self-repair는 다른 기능이 아니라 **같은 병(모델이 스키마를 모른다)의 사전 처방과 사후 처방**이다. 사전에 주면 첫 호출에 맞고, 사후에 고치면 두 번째 호출에서 맞는다. 그래서 재시도가 있는 시스템에서 스키마 카드가 사는 근거는 정확도가 아니라 **호출 수 33% 감소, 지연 25% 감소, 실패 경로 3분의 1**이다. 재시도가 없는 경로에서는 Δ +40pp의 정확도 차이가 그대로 남는다(이 문단의 수치는 전부 `qwen2.5:7b` 에서 잰 것이다).

이 결과는 내부 벤치의 「구조보존 큐레이션 Δ +51.0pp」를 **부정하지 않지만 조건을 붙인다**: 그 수치는 재시도 없는 단발 호출 조건의 값이다. 사업자 데이터도 같은 조건에서 같은 방향을 냈다 — 다만 **두 수의 모델이 다르다.** 위 표의 Δ +40pp(60% 대 20%)는 `qwen2.5:7b` 에서 잰 것이고, 정본 Δ +51.0pp는 현행 `qwen2.5-coder:7b` 값이다. 현행 모델로 같은 사업자 데이터를 재시도 없이 재면 7/10 대 1/10, 즉 **Δ +60pp**다. 세 수를 한 줄에 세워 「재현」이라 부르면 모델을 넘나든 비교가 되므로, 각 수의 모델을 붙여 읽어야 한다. 그리고 재시도를 허용하면 이득이 정확도에서 비용으로 이동한다 — **이것도 구 모델에서의 관측이고**, 현행 모델은 재시도를 붙여도 7/10 대 5/10으로 격차가 남는다(위 갱신 상자).
...
| 중국어 혼입 2건, 「dept_id 5번」 | 7B code-switching, id 그대로 노출 | 답변 프롬프트에 한국어 고정, 관계줄 해석, id 대신 이름, 동점 전부 나열. **2026-08-19: 모델을 `qwen2.5-coder:7b` 로 바꾼 뒤 한국어 고정 줄을 빼고 30문항을 다시 돌려 한자 혼입 0건을 확인했다 (재현: `ANSWER_LANG_LOCK=0 npm run companyx:ask`, 원자료 `eval/results/companyx-language-lock.json`)** — 이 모델에서는 원 결함이 재현되지 않는다. 그래도 그 줄은 유지한다: 30문항 한 번으로 「절대 안 샌다」를 말할 수 없고 비용이 0이며 실패 비용은 크다 |
...
- 재시도가 없을 때의 격차는 **Δ +60pp**(7/10 대 1/10)로 여전히 크다. 스키마 카드 없는 팔은 `qwen2.5-coder:7b` 5회 반복에서 예외 없이 1/10이었고, 옛 `qwen2.5:7b` 에서는 8회 반복에서 예외 없이 2/10이었다.
- **`qwen2.5:7b` 에서는** 재시도를 붙이면 격차가 **0~+10pp**로 사라지거나 한 문항 수준으로 줄어든다. 이 세션(2026-07-29)에서는 두 팔이 7/10으로 **정확히 같았다**. §0.6의 당시 결론이 다시 재현된 셈이다. **2026-08-19 에 생성 모델을 `qwen2.5-coder:7b` 로 바꾸고 다시 재니 이 결론은 더 이상 성립하지 않는다** — 재시도를 붙여도 7/10 대 5/10 으로 격차가 남는다(§0.6).
- 남는 차이는 비용이다 — **`qwen2.5:7b` 기준.** 같은 7/10을 스키마 카드는 **재시도 2회**로, 테이블명만은 **재시도 6회**로 달성한다. `qwen2.5-coder:7b` 에서는 애초에 같은 점수에 도달하지 못하고(7/10 대 5/10) 재시도는 **0회 대 7회**다.
...
**어떻게 만들었나.** PostgreSQL의 한국어 형태소 분석은 확장 설치와 superuser 권한을 요구해서 심사자의 재현 환경을 무겁게 만든다. 그래서 다른 길을 택했다. **정규화 계약을 앱에 정의하고(`air-server/src/normalize.ts`) 그 결과를 `simple` 설정의 tsvector에 넣는다.** 색인과 질의가 같은 함수를 통과하므로 계약이 코드로 남고 테스트로 검증된다.
...
**무엇을 만들었나.** `air-server/src/auditrecord.ts`는 파이프라인 결과를 감사 레코드로 바꾸는 **순수 변환**이다. 아무것도 실행하지 않는다.
...
| 파이프라인 지문 안정성 | **모델에 따라 다르다.** `qwen2.5:7b` 는 실행마다 흔들렸고(2026-08-18 세 번 관측: 4/5 · 3/5 · 5/5), `qwen2.5-coder:7b` 는 **세 번 다 5/5** 였다(2026-08-19). **세 번 같았다는 것은 세 번 같았다는 뜻이지 「항상」이 아니다** — 생성 모델이 끼는 경로라 원리적으로 보장이 아니다. 이 줄은 여전히 **관측**이며, 저장된 증거 파일은 마지막 실행값을 담는다 <!--metric-ok--> |
...
**그 함정이 실제로 작동한 기록, 그리고 그보다 나쁜 것 (2026-08-19).** 모델을 바꾸며
답변 30건을 나란히 놓고 비교하다 한 자리를 파고들었다.

| | 질문: 「Product-D1 제품과 관련된 **프로젝트**는?」 |
|---|---|
| 조건 | 같은 컨텍스트 1,174자 · 같은 레인 `graph` · 같은 후보(graph 140 / fused 42) |
| `qwen2.5:7b` | 「Client-Y, Client-AB, Client-B, …」 — 고객사 11개를 나열 |
| `qwen2.5-coder:7b` | 「주어진 정보로는 알 수 없습니다.」 |

**세 가지가 동시에 참이다.**

1. **답은 그래프에 있다.** 관련 프로젝트는 2홉(`Product-D1` ← `USES` ← client →
   `HAS_PROJECT` → project)이고 SQL 로 세면 **15개**가 닿는다.
   `graphExpand(depth 2)` 도 **`HAS_PROJECT` 엣지 16개를 실제로 낸다.**
2. **큐레이션이 그것을 버린다.** 최종 컨텍스트 1,174자에 `HAS_PROJECT` 가 **0건**이다 —
   1홉 `USES` 엣지가 예산을 전부 먹는다. **구조보존 큐레이션이 이 질문에서 졌다.**
   2홉 질문에서 답을 담은 엣지가 1홉 잡음에 밀리는 것은 우리 설계의 결함이고,
   이 문서의 논지가 자랑하는 바로 그 부품이다.
3. **개체 접지 검사는 그 실패를 못 봤다.** 옛 답은 컨텍스트에 있던 **고객사**를
   나열했고 — 질문이 요구한 개체 타입이 아니다 — 그 이름들이 컨텍스트에 있었으므로
   당시 접지율 **19/19 = 100%** 를 통과했다.

**「접지됐다」는 「맞다」가 아니다.** claim 단위 접지와 완전성을 따로 두는 이유가
이것이고, 그마저도 **질문이 요구한 개체 타입을 답이 맞췄는지**는 보지 않는다.

새 모델의 거부는 **주어진 컨텍스트 기준으로 옳다** — 없는 것을 지어내지 않았다.
접지 분모가 `19/19` → `18/18` 로 준 것은 품질 저하가 아니라 정당한 거부가 하나
늘어서다. 다만 **거부가 옳다는 것이 시스템이 옳다는 뜻은 아니다.**

지금 우리가 못 재는 것을 여기 적어 둔다 — 재지 않은 것을 잰 것처럼 두지 않는다.
...
| 운영 multiset 동등 (이 저장소 내부 지표) | 219/500 = 0.438 · 이전 표본 10/32 = 0.312 |
  ...
  **전수 500 기준으로 갈린 것은 24건**(공식 243 대 운영 219)이고 **8개 DB 에 걸쳐 있다** (card_games 5 · formula_1 4 · european_football_2 4 · toxicology 3 · student_club 3 · thrombosis_prediction 2 · debit_card_specializing 2 · financial 1). **24건 전부 distinct 집합은 gold 와 같다** — 예측이 중복 행을 많이 반환했을 뿐이라는 뜻이다(예: q173 financial, 예측 13행 vs gold 1행, distinct 1=1). 원자료 `eval/results/external-bird-rescore.json` 의 `diverged`. **2026-08-18 까지의 32문항 기록**에서는 갈린 것이 **1건**이었다(q345 `card_games`, 예측 3810행 vs gold 3행, distinct 3=3). gold 실행 실패 0건.
...
**2026-08-19 전수 측정.** 표집을 버리고 **Mini-Dev 500문항 전량**을 돌렸다(stride=1, 11/11 DB, 난이도 148/250/102 — 공식 구성 30/50/20 과 같다). 생성 모델도 `qwen2.5-coder:7b` 로 바꾼 뒤다. 공식 set 동등 **243/500 = 0.486**, 운영 multiset 동등 **219/500 = 0.438**. **표집 오차가 0이므로 「이 표본으로 추정할 수 없다」는 경고는 더 이상 필요 없다.** 다만 한 가지는 밝혀 둔다 — **Mini-Dev 데이터셋 자체가 500항목 중 `question_id` 137·138 을 중복 수록한다**(두 항목의 질문 텍스트가 같다). 고유 id 는 498이고 우리 500행은 데이터셋 항목을 그대로 덮은 것이다. 상류 데이터셋의 성질이라 고치지 않고 적어 둔다. `verify-bird-consistency` 가 우리 raw 의 id 다중집합을 **데이터셋과 직접 대조**한다. 대신 남는 한계를 적는다 — gold SQL 2건(q518, q701)이 하네스의 30초 실행 한도를 넘겨 **채점 불가**다. 둘 다 정상 질의이고 느릴 뿐이며(각각 101.6초, 240초에도 미완료) 한도가 없는 파이썬 재채점기는 500건을 전부 돌렸다. 하네스 헤드라인 43.6%는 BIRD 관행대로 그 2건을 오답으로 세고, 채점 가능분만 보면 43.8%다.
  ...
  **원인은 특정하지 않는다(2026-08-18 시점의 판단).** 7주 사이 스키마 카드·에러 처리·
  의존성이 여러 번 바뀌었고 어느 것이 6.3pp 를 만들었는지 우리는 모른다.
  재측정 자체가 늦은 것이 사실이며(`evidence-manifest` 가 이제 정본 증거의 나이를
  매번 출력한다), **그때의 32문항 표본은 아래 경고대로 대표성이 없었다.**
  2026-08-19 전수 500 으로 그 한계는 사라졌고, 남은 한계는 위 전수 문단이 적었다
  (gold 2건 시간 초과 · 상류 데이터셋의 중복 항목 · 도메인이 달라 내부 지표와 비교 불가).
...
- **외부 calibration:** BIRD Mini-Dev(SQLite) **전량 500문항**. **두 지표를 나눠 적는다** — 공식 set 동등 0.486와 이 저장소의 운영 multiset 동등 0.438(§5 참조, 2026-08-19 전수 측정). 헤드라인 별도 열이며 내부 수치와 비교 불가.
...
- **뒷받침 — self-correction 은 세 계열 전부에서 유의한, 거의 공짜인 이득이다.**
  우리 NL2SQL 재시도가 정확히 그 기법이다 — 다만 **`qwen2.5-coder:7b` 에서는 재시도가 점수를 올리지 않는다**(무재시도 7/10 · 재시도 7/10, 수리 0회). 옛 `qwen2.5:7b` 에서는 5/10 → 7/10 이었다.
...
- **위협 — schema linking 은 도움이 되지 않고, gold-table recall 96.5%의 임베딩 링커도
  ...
  변명하지 않고 **주장의 범위를 좁힌다.** 우리 ablation(1% / 37% / 88%, `qwen2.5-coder:7b`)은 사업자 스키마
...
| **실측 예** | 2026-08-18에 코드 60여 개 PR을 넣은 뒤 재측정 — **값 변화 0** | 파이프라인 지문이 08-18 에 4/5 → 3/5 → 5/5 로 흔들렸고, 모델을 바꾸자 08-19 에 세 번 다 5/5 였다. 내부 벤치는 7주 만에 83 → 81, 모델 교체로 88 |
...
## 11. 배점 대응표 (2026-08-20 공식 공지 기준)

출처 = 공식 공지 「심사 기준 및 배점 안내」(osscontest.kr/notice/41, 게시 2026-08-20). 각 항목에 대응하는 **이미 존재하는 증거**만 적는다. 없는 것은 없다고 적는다.

> 2026-08-22 갱신: 그 전까지 이 표는 OT 녹화 슬라이드(2026-07-23)를 근거로 삼았는데, 공식 공지가 2차 두 칸을 바꿨다 — **커뮤니티 확장 가능성 10 → 5, 오픈소스SW 적절성 10 → 15.** 나머지 항목과 합계(30 + 70)는 같다.
...
| 커뮤니티 확장 가능성 (5) | 품질관리 체계(테스트·평가 재현), 개발 로드맵, `CONTRIBUTING`·`CODE_OF_CONDUCT`·이슈/PR 템플릿·Discussions, good first issue 보존(#7), 비공개 취약점 신고 활성. **문은 열려 있으나 외부 참여(star·fork·외부 PR)는 아직 0** | 부분 |
| 오픈소스SW 적절성 (15) | 의존성 110개 전부 허용형, 카피레프트 0건, air/pgvector/Ollama 적재적소 사용. `docs/sbom.md`가 버전·라이선스·용도를 자동 생성하고 `scripts/sbom.mjs`가 드리프트를 CI 에서 막는다 | 있음 |
