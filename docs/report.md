...
## 0.14 홀드아웃 라우팅 평가 — in-sample 30/30의 진짜 의미

**왜 다시 쟀나.** 라우팅 30/30은 사업자가 공개한 예시 문항 기준이고, 라우터 어휘를 그 문항을 읽으며 작성했다. 즉 in-sample 수치다. 심사자가 "미공개 문항에서도 되나"라고 물으면 답할 수가 없었다. 그래서 라우터가 한 번도 보지 않은 문구로 물었다.

**평가셋이 공정한가가 먼저다.** 생성기 `build_holdout_route.py`가 30문항을 만들고 세 가지를 강제한다. 라벨은 평가자가 보고 맞춘 것이 아니라 **스키마와 온톨로지의 라우팅 신호**로 정한다. 공개 30문항에 나온 구체 문구는 그대로 베끼지 않는다. 검증을 통과하지 않으면 기록하지 않는다(처음에 2건이 걸려서 고쳤다). 고유명사는 데이터셋에 실재하는 것으로 다시 뽑는다.

**결과 (홀드아웃 30문항, `eval/results/companyx-holdout-route.json`).**

| 구분 | 수치 | 의미 |
|---|---|---|
| 결정적 히트 | **23/30 = 0.767** | 라우터가 정답 레인 하나를 골랐다 |
| 보수적 fan-out | 3/30 | 신호 부재로 양쪽을 병렬로 열었고, 정답 레인이 그 안에 있다 |
| 진짜 오류 | **4/30** | 라우터가 틀린 곳으로 본격 판단 |
| 커버율 | 0.867 | 히트 + fan-out 안에 정답이 있는 비율 |

**읽는 법 셋.**

1. **in-sample과의 격차는 30/30 대 0.767이다.** 이것이 과적합의 크기다. 공개 예시와 홀드아웃을 같은 숫자로 말하면 안 된다.
2. **fan-out을 오류로 세지 않았다.** 신호가 없을 때 양쪽을 여는 것은 라우터의 설계된 보수 동작이고, 그 안에 정답 레인이 들어 있다. 단, 그것을 정답으로 합치면 점수를 부풀리는 것이라 따로 둔다.
3. **진짜 오류 4건은 전부 knowledge_graph -> hybrid다.** "누구야", "관여하는"을 라우터가 관계 신호로 못 잡았다. 이것이 홀드아웃에서만 드러나는 실패다. 어휘를 이 문항에 맞춰 추가하면 홀드아웃이 in-sample이 되므로, 추가하지 않고 결과를 그대로 공개한다.

**혼동 행렬이 보여주는 것.** vector_search와 knowledge_graph 모두 hybrid로 흘렀다. structured -> ? 는 0건이다. 즉 관계형 라우팅은 튼튼하고, 관계형이 아닌 것을 가리는 경계가 흐리다는 뜻이다.

**결론.** 라우팅 30/30은 "공개 예시 30문항에서 30/30"으로 정정한다. 일반화 성능은 홀드아웃 30문항 기준 결정적 히트 0.767, 커버율 0.867로 별도 표기한다. 이 두 숫자를 섞지 않는다.
