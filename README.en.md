...
| Median latency | **9780 ms** | `docker compose` Ollama (CPU, no GPU passthrough), 30 questions end to end. On a GPU-backed host Ollama the same code runs at **799 ms**, roughly 13x faster |
| Fault injection | no-crash 4/4, partial 4/4, error-visible 4/4 | database stop, delay, partial failure |
| Tests | **388 assertions passing** | 313 offline + 75 requiring database and models; run in full via docker compose |
