...
MCP 표면은 도구 8종, 리소스 6종, 프롬프트 4종입니다. `audit.explain`은 한 질의를 끝까지 실행하고 **왜 그 답이 나왔는지**를 기계가 읽을 감사 레코드로 돌려줍니다. 라우팅 근거, 거부된 SQL과 사유, 융합에 합의한 소스, 정책 판정 다섯 가지, 접지 검사까지 한 레코드에 담깁니다. 리소스는 스키마 카드, 데이터셋 매니페스트, 평가 원자료 색인, 활성 프로파일, 감사 레코드 스키마, 증거 목록 여섯 가지를 도구 호출 없이 열람하게 합니다. 프롬프트는 서버가 실제로 쓰는 근거 기반 답변과 NL2SQL 템플릿을 그대로 노출합니다.
...
| NL2SQL 실행 일치 | **7/10**, 스키마 카드 제거 시 **1/10** | 재시도 없음. n=10, 세션 내 반복은 동일하고 세션 간에 1~2문항 흔들린다 | 정답 SQL의 실행 결과 |
| NL2SQL 실행 일치 (재시도 1회) | **7/10**, 스키마 카드 제거 시 **5/10** | 실패 SQL을 DB 카탈로그와 함께 1회 되먹임. **재시도 0회 대 7회** | 2026-08-19 재측정(qwen2.5-coder:7b) |
| **외부 calibration (BIRD Mini-Dev, 전량 500문항)** | 공식 set 동등 **243/500 = 0.486**, 운영 multiset 동등 219/500 = 0.438 | 11/11 DB, 난이도 148/250/102(공식 구성). 표집 오차 0 | **다른 데이터셋·다른 도메인이라 위 내부 수치와 비교할 수 없다.** gold 2건이 30초 실행 한도를 넘겨 채점 불가 |
...
| 응답 지연 중앙값 | **15746ms** | `docker compose` Ollama(CPU, GPU 패스스루 없음) 기준 종단 30문항. 반복 실측 9.8~18.5초로 변동이 크다. GPU가 붙은 호스트 Ollama에서는 **947ms**(원자료 `eval/results/companyx-ask-host-gpu.json`) | 실측 |  <!--metric:ask_median_ms-->
| 응답 지연 (호스트 GPU) | **947ms** | GPU가 붙은 호스트 Ollama(11434) 기준 같은 30문항, `qwen2.5-coder:7b`. 옛 `qwen2.5:7b` 는 같은 호스트에서 864ms 였다 — 모델도 세션도 다르므로 개선·악화로 읽지 않는다. 원자료 `eval/results/companyx-ask-host-gpu.json` | 실측 |  <!--metric:ask_median_ms_host--> <!--metric-ok-->
...
| 규칙 구간 결정론 | **5/5** (파이프라인 전체는 **모델에 따라 다르다** — `qwen2.5:7b` 는 4/5·3/5·5/5 로 흔들렸고 `qwen2.5-coder:7b` 는 세 번 다 5/5 였다. 모델이 SQL을 만드는 구간이라 **보장이 아니라 관측**이고, 세 번 같았다는 것은 「항상」이 아니다) | 같은 질의 2회 실행, 지문 비교 | 실측 |
...
| `verify-repo-presentation` | 저장소 페이지에 빈 탭·빠진 설명이 생기는 것 | 예 |
| `verify-repo-stats` | 배점 대응표의 커밋·PR·이슈 수가 실제보다 크게 뒤처지는 것 | 예 |

생성기 하나가 더 있습니다. `node scripts/sync-changelog.mjs` 는 **CHANGELOG.md 가 GitHub 릴리스 노트와 갈렸는지** 보고, `--write` 로 다시 만듭니다. 릴리스가 정본이고 CHANGELOG 는 파생이라 손으로 고치지 않습니다 — 클론한 사람이 GitHub 을 열지 않고도 버전 사이의 변화를 읽게 하려고 둡니다. 같은 이유로 `scripts/remeasure.mjs` 도 표에 없습니다 — 검사가 아니라 **재측정 도구**입니다. 결정론 평가 7종(라우팅·KG·벡터·홀드아웃 2종·다단계·장애)을 다시 돌려 **정본과 달라진 값만** 말합니다. 약 6분이고 대부분이 벡터 평가입니다. 생성이 끼는 `ask`·`bench:internal`·`external:bird` 는 값이 같기를 요구할 수 없어 **빼고 그 사실을 출력에 적습니다**.
...
1. **스키마 카드의 기여가 정확도인지 비용인지는 생성 모델에 달렸습니다.** 값 어휘까지 담은 스키마 카드는 재시도가 없을 때 **7/10 대 1/10**으로 갈립니다. `qwen2.5:7b`(2026-07)에서는 실패한 SQL을 데이터베이스 카탈로그와 함께 한 번 되먹이면 7~8/10 대 7/10으로 **좁혀졌고** 이득이 재시도 횟수(2회 대 6회)로 옮겨갔습니다. 그런데 `qwen2.5-coder:7b`(2026-08-19)로 바꾸고 다시 재니 재시도를 붙여도 **7/10 대 5/10**으로 격차가 남고 재시도는 **0회 대 7회**입니다 — 스키마 카드를 주면 이 모델은 수리가 아예 필요 없습니다. 즉 「재시도가 있으면 스키마 카드는 비용만 산다」는 결론은 **그 모델에서만 참**이었습니다. 범위로 적는 이유는 16회 반복 실측에서 **세션 안에서는 결과가 완전히 동일한데 세션이 바뀌면 1~2문항이 움직였기** 때문입니다. 단일 실행 수치를 헤드라인으로 쓰지 않습니다.
...
- **커뮤니티 지표가 0입니다.** star 0 · fork 0 · **외부 기여자의 PR 0건**입니다. 이슈 템플릿·기여 가이드·good first issue·Discussions 를 갖췄고 커뮤니티 프로필 health 는 100% 이지만, **실제로 밖에서 들어온 기여는 아직 없습니다.** 지금까지의 PR 은 전부 저희 것입니다.
- **외부 벤치마크에서는 내부 점수보다 한참 낮습니다.**
  - 내부 100문항 execution-match **88/100**
  - BIRD Mini-Dev 전량 500문항 공식 set 동등 **243/500 = 0.486**
  - 데이터셋도 도메인도 달라 직접 비교할 수는 없지만, **내부 수치가 이 시스템의 일반적 실력이 아니라는 뜻**으로 읽어 주십시오. 내부 문항은 저희가 만들었고 BIRD 는 남이 만들었습니다.
