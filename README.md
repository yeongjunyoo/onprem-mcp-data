...
| 테스트 | **412단언 통과** | 오프라인 285 + DB·모델 통합 127. 데이터셋 없는 CI에서는 오프라인 274(사업자 데이터셋이 있어야 도는 온톨로지 커버리지 단언 11건 제외). CI가 러너 출력에서 다시 세어 대조한다. 원자료 `eval/results/test-counts.json` | 실측 |  <!--metric:test_total-->
