...
| 응답 지연 중앙값 | **13527ms** | `docker compose` Ollama(CPU, GPU 패스스루 없음) 기준 종단 30문항. 반복 실측 9.8~18.5초로 변동이 크다. GPU가 붙은 호스트 Ollama에서는 **864ms**(원자료 `eval/results/companyx-ask-host-gpu.json`) | 실측 |  <!--metric:ask_median_ms-->
...
| 테스트 | **394단언 통과** | 오프라인 267 + DB·모델 통합 127. `npm run test:integration` 포함 전량 실행. 집계 원자료 `eval/results/test-counts.json` | 실측 |  <!--metric:test_total-->
